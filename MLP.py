...
def calculate_risk_score(row):
    risk_factors = 0
    if row["Age"] > 60: risk_factors += 1
    if row["Blood Pressure (mmHg)"] > 140: risk_factors += 1
    if row["Cholesterol (mg/dL)"] > 240: risk_factors += 1
    if row["Has Diabetes"] == "Yes": risk_factors += 1
    if row["Smoking Status"] == "Current": risk_factors += 1
    if row["Chest Pain Type"] == "Type 1": risk_factors += 1
    return risk_factors
...
gender = "Male" if input("Seçiminiz: ") == "0" else "Female"
age = int(input("Yaş: "))
bp = float(input("Tansiyon (mmHg): "))
chol = float(input("Kolesterol (mg/dL): "))
print("Diyabet Durumu:\n0 - No\n1 - Yes")
diabetes = "No" if input("Seçiminiz: ") == "0" else "Yes"
print("Sigara Durumu:\n0 - Never\n1 - Former\n2 - Current")
smoke = ["Never", "Former", "Current"][int(input("Seçiminiz: "))]
print("Göğüs Ağrısı Tipi:\n0 - Type 1\n1 - Type 2\n2 - Type 3\n3 - Type 4")
cp_type = f"Type {int(input('Seçiminiz: ')) + 1}"
...
for name, model in models.items():
    ...
    p = precision_score([true_class], [pred], average='weighted', zero_division=0)
    r = recall_score([true_class], [pred], average='weighted', zero_division=0)
    f = f1_score([true_class], [pred], average='weighted', zero_division=0)
    print(f"{name:20s} | Tahmin: {pred:6s} | Precision: {p:.2f} | Recall: {r:.2f} | F1: {f:.2f}")
...
for i in range(len(methods)):
    plt.text(i - 0.15, accuracy[i]*100 + 0.5, f"{accuracy[i]*100:.2f}%", size='large')
